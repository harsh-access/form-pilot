...
import { DynamicForm, Section, SubSection, FormField } from 'src/app/view-modal/DynamicFormModel';
import { IFileUploadContainer, ILkpOptionArr, IPropList } from 'src/app/view-modal/GeneralModels';
import { getFieldsetPropertyList, OnboardedObjectTypes, prepareDynamicFormGroup, whitespaceValidator } from './fieldset-propertyList-functions';
...
export class FieldsetSidebarComponent implements OnInit {
  ...
  @Input() stage !: any; // Simplified for POC
  @Input() section !: any; // Simplified for POC  
  @Input() field !: any; // Simplified for POC
  ...
  uniqueStr: string = "1";
  currentInductionSection: any; // Simplified for POC
  initialTicNlicenseValue: any;
  excludeAtsPropertyValue: string[] = [];
  isDependentDataChanges: boolean = false;
  allColumns: any[] = []; // Simplified for POC

  constructor(private fb: FormBuilder, private fileService: FileService, private dynamicFormService: DynamicFormService, private fieldsetService: FieldsetService) { }
  ...
  private setValuesToForm() {
    ...
    this.excludeAtsPropertyValue = []; // ATS integration disabled for simplified POC

    if (!this.isAddFieldset) {
      if (this.fieldsetCategory == 1) {
        const bulkSkillDocIndex = this.propList.findIndex(x => x.prop == 'bulkSkillDoc');
        if (bulkSkillDocIndex > -1) {
          this.propList.splice(bulkSkillDocIndex, 1);
        }
      }
      if (this.fieldsetCategory == 2) {
        const inductionTypeIndex = this.propList.findIndex(x => x.prop == 'inductionType');
        if (inductionTypeIndex > -1) {
          this.propList.splice(inductionTypeIndex, 1);
        }
      }
    }
  ...
  private async setIntegrationList(integrationType: any, IsApplyFilter: boolean = true) {
    // ATS integration disabled for simplified POC
    return;
  }
  ...
  getIndQueSetGroup(inductionType: string): any {
  ...
  async saveTicketNLicence(): Promise<{ isSuccess: boolean, qry: object }> {
    ...
    processedData = { isSuccess: false, errMsg: "Bulk upload not supported in simplified POC", qry: {} };
    if (!processedData.isSuccess) {
      console.error(processedData.errMsg);
      return { isSuccess: false, qry: null };
    }
  ...
  private getControlType(refFormFieldId: number): any {
    // Simplified for POC
    return 11; // Default to TextBox
  }
