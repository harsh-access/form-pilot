...
export class SidebarPropFormgroupComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private toast: RxToast, private fileService: FileService, private fieldsetService: FieldsetService, private dynamicFormService: DynamicFormService) {
  ...
  getDependent(prop: string): boolean {
    ...
    return dependency.refValue.contains(this.FormGroupToDisplay.value[dependency.id])
  }
  ...
  removeFieldOption(prop: string, idx: number) {
    ...
    if (options.length == 2) {
      this.toast.show('At least two options are required', { status: 'error' });
      return;
    }
    // else
    ...
    if (removedOptionId == selectedOptionBoxId) {
      ...
      ids.removeAt(ids.findIndex(x => x == selectedOptionBoxId));
  ...
  removeNestedSet(prop: string, idx: number) {
    ...
    if (atLeastOneSet.indexOf(prop) > -1 && (<FormArray>this.FormGroupToDisplay.get(prop)).length == 1) {
      this.toast.show('At least one set is required', { status: 'error' });
      return;
    }
  ...
  downloadTicketLicencesData() {
    this.fieldsetService.getExcelSheet(OnboardedObjects.LicencesandCertificates);
  }
  ...
  downloadFile(prop: string) {
    ...
    if (url && url != null && url != '') {
      ...
      this.toast.show('Right now your file changes are not saved');
    }
    else {
