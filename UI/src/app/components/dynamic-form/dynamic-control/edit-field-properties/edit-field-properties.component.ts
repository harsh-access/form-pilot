...
import { ATSDetailsForExcel, ATSMappingResponse, ATSProperty, Columns, IntegrationLists, Rows, SubSection } from 'src/app/view-modal/DynamicFormModel';
...
export class EditFieldPropertiesComponent implements OnInit {
  ...
  @Output() updatePropChanges = new EventEmitter<{ isNewAndCanceled?: boolean; isDependencyCRUD?: boolean; field?: FormField; changes: string[]; }>();
  ...
  constructor(private fb: FormBuilder, private toast: RxToast, private popup: RxPopup, private fieldIntegrationService: FieldIntegrationService, private dynamicFormService: DynamicFormService, private fileService: FileService, private fieldsetService: FieldsetService, private _atsLookupService: AtsLookupService) {
  ...
  ngOnInit(): void {
    ...
    this.openCanvas();
    this.listenToMergeTagTextBoxChanges();
    this.listenToMergeTagRadioButtonChanges();
  ...
  downloadSkillData() {
    this.fieldsetService.getExcelSheet(OnboardedObjects.Skills);
  }

  setAtsIntegrationList(controlType: string) {
    let arrayList: Array<{ id: number; value: string }> = [];
    let arrayListCore: Array<{ id: number; value: string }> = [];

    let allowedDataTypes = this.fieldIntegrationService.fieldTypeMappings[FieldTypeEnum[controlType]] || [];
    let allowedDataTypesCore = this.fieldIntegrationService.fieldTypeMappingsForCoreField[FieldTypeEnum[controlType]] || [];

    this.multilineAtsArray = this.fieldIntegrationService.IntegrationInfo.atsPropertyList.filter(data => data.atsDataType == ATSPropertyDatatypeEnum.Multiline && data.objectType == OnboardedObjects.Candidate).map((Data) => Data.atsPropertyId);

    // Use allowedDataTypes as needed
    this.excludeAtsPropertyId = Array.from(new Set(
      this.fieldIntegrationService.columns
        .filter(column => column.atsPropertyId && column.atsPropertyId !== this.field?.atsPropertyId && !this.multilineAtsArray.includes(column.atsPropertyId))
        .map(column => column.atsPropertyId)
    ));
    ...
    arrayList = this.fieldIntegrationService.IntegrationInfo.atsPropertyList.filter(atsItem => allowedDataTypes.includes(atsItem.atsDataType) && atsItem.objectType == OnboardedObjects.Candidate && !this.excludeAtsPropertyId.includes(atsItem.atsPropertyId)).map(item => ({ id: item.atsPropertyId, value: item.atsLabel }));
    arrayListCore = this.fieldIntegrationService.IntegrationInfo.atsPropertyList.filter(atsItem => allowedDataTypesCore.includes(atsItem.atsDataType) && atsItem.objectType == OnboardedObjects.CoreFields && !this.excludeAtsPropertyId.includes(atsItem.atsPropertyId)).map(item => ({ id: item.atsPropertyId, value: item.atsLabel }));

    // console.log( this.fieldIntegrationService.IntegrationInfo.atsPropertyList,allowedDataTypes, this.excludeAtsPropertyId, this.excludeAtsPropertyValue,arrayList);



    if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Bullhorn) {
      this.setIntegrationList(OnboardedObjects.Candidate, [], "ATS / CRM Fields", false);
      //For Customfield
      this.setIntegrationList(OnboardedObjects.CustomFields, this.fieldIntegrationService.fieldTypeMappingsForCustomField[FieldTypeEnum[controlType]] || [], undefined, false);
    }
    else if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Rdb || this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.ARCRM) {
      this.setIntegrationList(OnboardedObjects.Candidate, this.fieldIntegrationService.fieldTypeMappings[FieldTypeEnum[controlType]] || [], "ATS / CRM Fields", true);
      //For Customfield
      this.setIntegrationList(OnboardedObjects.CustomFields, this.fieldIntegrationService.fieldTypeMappingsForCustomField[FieldTypeEnum[controlType]] || [], undefined, true);
    }
    else if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Vincere) {
      if (this.field.objectName == String(OnboardedObjects.CoreFields)) {
        this.setValueIntoPropList(OnboardedObjects.CoreFields, "atsPropertyId", arrayListCore, "ATS / CRM Fields");
      }
      else {
        this.setValueIntoPropList(OnboardedObjects.Candidate, "atsPropertyId", arrayList, "ATS / CRM Fields");
      }
      this.setIntegrationList(OnboardedObjects.CustomFields, this.fieldIntegrationService.fieldTypeMappingsForCustomField[FieldTypeEnum[controlType]] || []);
    }
    else {
      this.setValueIntoPropList(OnboardedObjects.Candidate, "atsPropertyId", arrayList);
      //For Customfield
      this.setIntegrationList(OnboardedObjects.CustomFields, this.fieldIntegrationService.fieldTypeMappingsForCustomField[FieldTypeEnum[controlType]] || []);
    }
  ...
  setupCommonValuesToForm(field: FormField) {
    ...
    // Work For Skills Section && Single Choice Section for Merge Tags
    if (this.currentCat.catId == 8 || this.currentCat.catId == 3) {
  ...
  private setValuesToForm(field: FormField) {
    if (this.isEditFormIntegration && this.formType != 171) {
      this.tabsToShow.push('Integration');
    }

    if ((this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Rdb || this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.ARCRM) && (field.objectName == "customskill" || (field.atsPropertyValue != null && field.atsPropertyValue.toUpperCase().startsWith('NOTEBOOK.')))) {
      if (this.tabsToShow.includes('Integration')) {
        this.tabsToShow.remove('Integration')
      }
    }
    ...
    try {
      ...
      if (this.isSupportUser) {
        this.EditForm = this.setDynamicValidation(this.EditForm, this.propList);
      }

      // check for MergeTag Validations
      this?.enableMergeTagValidation(this.isSupportUser);
      ...
      this.EditForm.get('controlType').valueChanges.subscribe(controlType => {
        ...
        switch (FieldTypeEnum[field.controlType]) {
          ...
          case FieldTypeEnum.NumericTextBox:
          case FieldTypeEnum.DatePicker:
          case FieldTypeEnum.ContactNumber:
          ...
          case FieldTypeEnum.SpecialLable:
            break;
          default:
      ...
      this.EditForm.get('custTypeIsCustomfieldIntegration').valueChanges.subscribe(IsCustomfield => {
        ...
        if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Vincere && this.field?.objectName == (OnboardedObjects[OnboardedObjects.CoreFields]) && iscustomfield) {
          this.EditForm.patchValue({ custTypeIsCorefieldIntegration: false })
        }
        else if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Vincere && this.field?.objectName == (OnboardedObjects[OnboardedObjects.CoreFields]) && !iscustomfield) {
          this.EditForm.patchValue({ custTypeIsCorefieldIntegration: true })
        }
      ...
      if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.JobAdder) {
        this.atsPropertyIdSubscription = this.EditForm.get('atsPropertyId').valueChanges.pipe(distinctUntilChanged()).subscribe(atsPropertyId => {
          if (atsPropertyId) {
            this.field.atsPropertyId = atsPropertyId;
            if (this.isNotesProperty(atsPropertyId)) {
              this.field.atsPropertyId = atsPropertyId;
              this.setIntegrationList(OnboardedObjects.Candidate, [], "Notes Type", false);
            }
            else {
              let indexToSkillUpdate = this.propList.findIndex(obj => obj.propLabel === "Notes Type");
              if (indexToSkillUpdate !== -1) {
                this.propList[indexToSkillUpdate].attr.listArr = [];
                this.EditForm.patchValue({ custTypeAtsPropertyValue: null, atsPropertyValue: null })
              }
            }
  ...
  isNotesProperty(atsPropertyId: number): boolean {
    const notesProperty = this.fieldIntegrationService.IntegrationInfo.atsPropertyList.find(property => property.atsPropertyId === atsPropertyId && property.atsLabel === "Notes");
    return notesProperty !== undefined;
  }

  private manageCustomfieldIntegration1(IsCustomfield) {

    if (!IsCustomfield) {
      if (!this.fieldIntegrationService.IntegrationInfo.atsPropertyList.filter(atsData => atsData.objectType == OnboardedObjects.Candidate).includes(this.EditForm.value.atsPropertyValue)) {
        this.EditForm.patchValue({
          atsPropertyId: null
        });
      }
    ...
    if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.JobAdder) {
      if (IsCustomfield) {
        ...
        if (indexToSkillUpdate !== -1) {
          this.propList[indexToSkillUpdate].attr.listArr = [];
          this.EditForm.patchValue({ custTypeAtsPropertyValue: null })
        }
      } else {
        if (this.isNotesProperty(this.EditForm.value.atsPropertyId) || this.isNotesProperty(this.field.atsPropertyId)) {
          this.setIntegrationList(OnboardedObjects.Candidate, [], "Notes Type", false);
        } else {
          let indexToSkillUpdate = this.propList.findIndex(obj => obj.propLabel === "Notes Type");
          if (indexToSkillUpdate !== -1) {
            this.propList[indexToSkillUpdate].attr.listArr = [];
            this.field.atsPropertyValue = null;
            this.EditForm.patchValue({ custTypeAtsPropertyValue: null })
          }
        }
      }
    ...
    if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Bullhorn || this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Rdb || this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.ARCRM) {
      if (IsCustomfield) {
        let indexToSkillUpdate = this.propList.findIndex(obj => obj.propLabel === "ATS / CRM Fields" && (!obj?.propIntegrationType || obj.propIntegrationType.contains(IntegrationType[this.fieldIntegrationService.IntegrationInfo.integrationType])));
        if (indexToSkillUpdate !== -1) {
          this.propList[indexToSkillUpdate].attr.listArr = [];
          var IsApplyFilter = false;
          if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Rdb || this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.ARCRM) {
            IsApplyFilter = true;
          }
          this.setIntegrationList(OnboardedObjects.CustomFields, this.fieldIntegrationService.fieldTypeMappingsForCustomField[FieldTypeEnum[this.field.controlType]] || [], undefined, IsApplyFilter);
          this.EditForm.patchValue({ custTypeCandidateAtsPropertyValue: null, atsPropertyValue: null })
        }
      } else {
        if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.Rdb || this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.ARCRM) {
          this.setIntegrationList(OnboardedObjects.Candidate, this.fieldIntegrationService.fieldTypeMappings[FieldTypeEnum[JSON.parse(
            JSON.stringify(this.intialFormValue['controlType'])
          )]] || [], "ATS / CRM Fields", true);
  ...
  private manageCorefieldIntegration(IsChanged) {
    ...
    this.propList.forEach(prop => {
      if (prop?.propCategory != null && prop?.propCategory.split(',').includes(OnboardedObjects[OnboardedObjects.CoreFields]) && (!prop?.propIntegrationType || prop.propIntegrationType.contains(IntegrationType[this.fieldIntegrationService.IntegrationInfo.integrationType]))) {
    ...
    if (IsChanged && iscorefield) {
      ...
      this.excludeAtsPropertyId = Array.from(new Set(
        this.fieldIntegrationService.columns
          .filter(column => column.atsPropertyId && column.atsPropertyId !== this.field?.atsPropertyId)
          .map(column => column.atsPropertyId)
      ));

      let allowedDataTypes = this.fieldIntegrationService.fieldTypeMappingsForCoreField[FieldTypeEnum[this.field.controlType]] || [];
      arrayList = this.fieldIntegrationService.IntegrationInfo.atsPropertyList.filter(atsItem => allowedDataTypes.includes(atsItem.atsDataType) && atsItem.objectType == OnboardedObjects.CoreFields && !this.excludeAtsPropertyId.includes(atsItem.atsPropertyId)).map(item => ({ id: item.atsPropertyId, value: item.atsLabel }));
  ...
  private manageCustomfieldIntegration() {
    if (this.isAddField) {
      ...
      if (this.fieldIntegrationService.IntegrationInfo.integrationType == IntegrationType.JobAdder) {
        if (this.isNotesProperty(this.field.atsPropertyId)) {
          this.setIntegrationList(OnboardedObjects.Candidate, [], "Notes Type", false);
        }
      }
    }
  ...
  private setValueIntoPropList(integrationType: OnboardedObjects, objectProp: string, arrayList: Array<{ id: number, value: string, type?: string }>, propLabel?: string) {
    this.propList.forEach(prop => {
      if (prop?.propCategory != null && prop?.propCategory.split(',').includes(OnboardedObjects[integrationType]) && (!prop?.propIntegrationType || prop.propIntegrationType.contains(IntegrationType[this.fieldIntegrationService.IntegrationInfo.integrationType]))) {
        prop.propIsVisible = true;
      }
    });
    ...
    if (propLabel) {
      indexToSkillUpdate = this.propList.findIndex(obj => obj.propLabel === propLabel && (!obj?.propIntegrationType || obj.propIntegrationType.contains(IntegrationType[this.fieldIntegrationService.IntegrationInfo.integrationType])));
    } else {
      indexToSkillUpdate = this.propList.findIndex(obj => obj.prop === objectProp && obj?.propCategory.split(',').includes(OnboardedObjects[integrationType]) && (!obj?.propIntegrationType || obj.propIntegrationType.contains(IntegrationType[this.fieldIntegrationService.IntegrationInfo.integrationType])));
    }

    if (indexToSkillUpdate !== -1) {
      this.propList[indexToSkillUpdate].attr.listArr = arrayList;

      //Updating the Rx-select Box
      switch (this.fieldIntegrationService.IntegrationInfo.integrationType) {
        case IntegrationType.JobAdder:
          this.EditForm.patchValue({ custTypeAtsPropertyValue: this.field.atsPropertyValue })
          break;
        case IntegrationType.Rdb:
        case IntegrationType.ARCRM:
          if (this.isCustomfieldIntegration) {
            this.EditForm.patchValue({ atsPropertyValue: this.field.atsPropertyValue })
          } else {
            this.EditForm.patchValue({ custTypeCandidateAtsPropertyValue: this.field.atsPropertyValue })
          }
          break;
        ...
        case IntegrationType.Vincere:
          if (this.currentCat.catId === 8) {
            const propIndex = this.propList.findIndex(item => item.prop === 'atsPropertyValue' && item.propCategory === 'Skills');
            const categoryItem:any = arrayList?.find((item:any) => this?.field?.atsPropertyValue === item?.id);
            this.propList[propIndex].attr.listArr = arrayList?.filter((skill:any) => skill?.category === categoryItem?.category);
            if(categoryItem?.category === 'functional') {
              this.EditForm.patchValue({ custTypeSkillCategory: true });
            }else{
              this.propList[propIndex].attr.listArr = arrayList?.filter((skill:any) => skill?.category === 'industry');
            }
          } else {
            this.EditForm.patchValue({ atsPropertyValue: this.field.atsPropertyValue, atsPropertyId: this.field.atsPropertyId })
          }
  ...
  private setDependency() {
    ...
    const isAlreadyParantField = this.section.rows.flatMap(x => x.columns)
      .findIndex(t => t.dependentFormFieldColumns.length > 0 && t.dependentFormFieldColumns[0].refFormFieldId == this.field.id) != -1;
    ...
    // console.log('setDrpList', this.field.id, columnList, drpList);
  ...
  isEnableColumnIdex_MaxColumn() {
    ...
    if (!this.isAddField) {
      ...
      var FirstDataField: FormField | Error = this.section.rows.flatMap((x) => x.columns).sort((a, b) => a.tabIndex - b.tabIndex).firstOrDefault();
      if (FirstDataField['formFieldId'] == this.field.id) {
        if (columnIndexControlType !== -1) {
  ...
  private manageCat8(isFormToField: boolean = false) {
    if (this.fieldIntegrationService.IntegrationInfo.integrationType != IntegrationType.StandAlone && (this.isAddField || [...this.onboardedObjectsStrArr, "skill"].contains(this.field?.objectName))) {
      this.tabsToShow = ['Settings', 'Advance', 'Integration'];
    } else {
  ...
  setDepencyStatus(prop: string, propValue: any) {
    ...
    if (!propValue['dependencyStatus']) {
      ...
      obj['refValue'] = obj['refFormFieldId'].find((y: FormField) => y.formFieldId == this.field.dependentFormFieldColumns[0].refFormFieldId).lookUps;
    }
  ...
  getDependent(prop: string): boolean {
    ...
    return dependency?.refValue?.contains(this.EditForm.value[dependency.id]);
  }
  ...
  onFileChange(data: any, item: IPropList) {
    ...
    if (file) {
      ...
      {
        this.toast.show("File size is larger than 5000 kb. It is not allowed", { status: 'error' });
      ...
      if (extensionsArray.indexOf(ext) == -1) {
        this.toast.show("File type not supported", { status: 'error' });
        this.EditForm.value[item.prop] = this.field[item.prop];
        return;
      }
      ...
      reader.onload = (readerEvt: any) => {
        ...
        if (fileBase64 != null && fileBase64 != '' && apiFileData.fileName != null && apiFileData.fileName != '') {
          ...
          if (!this.isAddField) apiFileData['formFieldId'] = this.field.id;
  ...
  setDropDownValue(field: FormField) {
  ...
  fallBackIntegration(field: FormField) {
    if ([1, 2, 3, 4, 6].includes(this.currentCat.catId)) {
      this.setAtsIntegrationList(field.controlType)
      this.manageCustomfieldIntegration()
    }
  ...
  downloadFile(prop: string) {
    ...
    if (url && url != null && url != '') {
      ...
      this.toast.show('Save the changes first before you can download');
    }
    else {
      this.toast.show('File Not available', { status: 'error' });
    }
  ...
  private staticSetupByControlType() {
    switch (this.EditForm.value['controlType']) {
      case 'DocumentCollection':
        ...
        if (this.fieldIntegrationService.IntegrationInfo.isEditFormIntegration) {
          this.EditForm.value.objectName = String(OnboardedObjects.Documents);
          this.EditForm.value.atsPropertyId = this.fieldIntegrationService.IntegrationInfo.docAtsPropertyId;
      ...
      case 'SwitchCheckBox':
        ...
        //Integration Related Work
        if (this.fieldIntegrationService.IntegrationInfo.isEditFormIntegration) {
          this.EditForm.value.objectName = String(OnboardedObjects.Skills);
          this.EditForm.value.atsPropertyId = this.fieldIntegrationService.IntegrationInfo.skillAtsPropertyId;
          this.changesArr.push('atsPropertyId');
        } else {
          this.EditForm.value.objectName = 'skill';
        }
  ...
  private manage_Cand_Cus_FieldFlow() {
    if (this.fieldIntegrationService.IntegrationInfo.isEditFormIntegration) {
      if (this.EditForm.value.custTypeIsCustomfieldIntegration) {
        ...
        this.field.atsPropertyId = this.fieldIntegrationService.IntegrationInfo.customFieldAtsPropertyId;
        this.EditForm.value.atsPropertyId = this.fieldIntegrationService.IntegrationInfo.customFieldAtsPropertyId;
        this.changesArr.push('atsPropertyId');
      }
        ...
        this.field.atsPropertyValue = this.fieldIntegrationService.IntegrationInfo.atsPropertyList.find(property => property.atsPropertyId === this.field.atsPropertyId)?.apiProperty || null;
  ...
  private async uploadfiles() {
    ...
    for (let idx = 0; idx < this.filesContainer.length; idx++) {
      ...
      if (response && response.isSuccess == true)
        ...
        this.toast.show(response.message, { status: 'error' });
        this.EditForm.value[item.prop] = this.field[item.prop];
      }
    }
    this.filesContainer = [];
  ...
  async saveChanges() {
    if (this.EditForm.invalid || !this.validateSave()) {
      this.toast.show('Please fill required value', { status: 'error' });
      return;
    }
    ...
    //This Work is only For BulkSkill Upload
    if (this.isAddField && this.filesContainer.length > 0 && this.currentCat.catId == 8) {
      let processedData = this.fieldsetService.processBulkTicketNLicence(this.filesContainer[0].fileData.base64Data, this.section.formSectionId, this.section.rows.at(-1).columns.at(-1).tabIndex + 1, this.EditForm.value.isRequired, true);
      if (processedData.isSuccess) {
        this.updateFieldsetPropChanges.emit({ isSuccess: true, qry: processedData.qry });
      } else {
        this.toast.show(processedData?.errMsg ?? 'Something went wrong', { status: 'error' });
        return;
      }
    } else {
  ...
  private confirmAtsMappingChange() {
    return this.popup.show(YesNoPopupComponent, {
      bodyMessage: 'Do you want to sync field options from your ATS/CRM? This will match the options in Onboarded.',
      noteMessage: "Note - If you do not match the options as per ATS/CRM, the integration will not work!",
      buttonUIProps: {
        yes: { text: 'Yes', color: 'bg-primary bg-opacity-75 custom-blue-bg' },
        no: {
          text: 'Cancel', color: 'custom-red-bg'
        }
      }
    });
  }
  ...
  private handleAtsPropertyValueChange(event: any) {
    ...
    // if catId in 2, 3, 4 (List Box, Single Choice, Multiple Choice)
    if (this.isAtsLookupGenerationEnabled) {
      this.confirmAtsMappingChange().then(t => {
        if (t.isConfirm) {
          switch (this.fieldIntegrationService.IntegrationInfo.integrationType) {
            case IntegrationType.Vincere:
              {
                this.setVincereCustomFieldLookups(event?.item?.key?.split(":")[1]);
              }
              break;
            case IntegrationType.Bullhorn:
              {
                this.setBullhornCustomFieldLookups(event?.sourceItem);
              }
              break;
            default:
              console.warn('Unsupported integration type for ATS Property Value');
              break;
          }
        }
        else {
    ...
    try {
      if (key) {
        this._atsLookupService.getVincereCustomFieldLookups(key).subscribe((res) => {
          ...
          if (!res.success) {
            this.toast.show('Error fetching Vincere Custom Field Lookups', { status: 'error' });
            return;
          }

          if (res.values && res.values.length == 0) {
            this.toast.show('No options found for the selected ATS Property value', { status: 'error' });
            return;
          }
    ...
    try {
      if (Item?.dataSourceUrl !== null && Item?.options === null) {
        this._atsLookupService.getBullhornCustomFieldLookups(Item.dataSourceUrl).subscribe((res) => {
          ...
          if (!res.success) {
            this.toast.show('Error fetching Bullhorn Custom Field Lookups', { status: 'error' });
            return;
          }
      ...
      this.confirmAtsMappingChange().then(t => {
        if (t.isConfirm) {
          switch (this.fieldIntegrationService.IntegrationInfo.integrationType) {
            case IntegrationType.Vincere:
              {
                this.setVincereCandidateCoreFieldLookups(event?.item?.key);
              }
              break;
            default:
              console.warn('Unsupported integration type for ATS Property Value');
    ...
    try {
      if (key) {
        this._atsLookupService.getVincereCandidateCoreFieldLookups(key).subscribe((res) => {
          ...
          if (!res.success) {
            this.toast.show('Error fetching Vincere Candidate/Core Field Options', { status: 'error' });
            return;
          }

          if (res.values && res.values.length == 0) {
            this.toast.show('No options found for the selected ATS Property', { status: 'error' });
            return;
          }
  ...
  private get isAtsLookupGenerationEnabled(): boolean {
    return this.isInProjectClient &&
      ...
      [IntegrationType.Vincere.toString(), IntegrationType.Bullhorn.toString()].includes(this.fieldIntegrationService.IntegrationInfo.integrationType);
  }

  get isFieldMapped(): boolean {
    switch (this.fieldIntegrationService.IntegrationInfo.integrationType) {
      case IntegrationType.Vincere: {
        let atsPropertyIdValue = this.EditForm.get('atsPropertyId')?.value;
        let atsPropertyValue = this.EditForm.get('atsPropertyValue')?.value;
        let isCustomfieldIntegration = this.EditForm?.value?.custTypeIsCustomfieldIntegration;
        return (atsPropertyIdValue !== null && atsPropertyIdValue !== '') || (isCustomfieldIntegration && atsPropertyValue !== null && atsPropertyValue !== '');
      }
      case IntegrationType.Bullhorn: {
        let custTypeCandidateAtsPropertyValue = this.EditForm.get('custTypeCandidateAtsPropertyValue')?.value;
        let atsPropertyValue = this.EditForm.get('atsPropertyValue')?.value;
        let isCustomfieldIntegration = this.EditForm?.value?.custTypeIsCustomfieldIntegration;
        return (custTypeCandidateAtsPropertyValue !== null && custTypeCandidateAtsPropertyValue !== '') || (isCustomfieldIntegration && atsPropertyValue !== null && atsPropertyValue !== '');
      }
      default:
        return false;
    }
  ...
  public regenarateAtsLookups(prop: string) {
    this.confirmAtsMappingChange().then(t => {
      if (t.isConfirm) {
        switch (this.fieldIntegrationService.IntegrationInfo.integrationType) {
          case IntegrationType.Vincere: {
            if (this.EditForm.get('custTypeIsCustomfieldIntegration')?.value) {
              this.setVincereCustomFieldLookups(this.EditForm.get('atsPropertyValue')?.value?.split(":")[1]);
            }
            else {
              this.setVincereCandidateCoreFieldLookups(this.EditForm.get('atsPropertyId')?.value);
            }
            break;
          }
          case IntegrationType.Bullhorn: {
            const storageKey = `Client_${this.clientId}`;
            const integrationList = JSON.parse(localStorage.getItem(storageKey) || '{}') as IntegrationLists;
            let selectedKey = null;
            let matchedField = undefined;
            if (this.EditForm.get('custTypeIsCustomfieldIntegration')?.value) {
              selectedKey = this.EditForm.get('atsPropertyValue')?.value
              matchedField = integrationList?.customFieldList?.find(field => field.id === selectedKey);
            } else {
              selectedKey = this.EditForm.get('custTypeCandidateAtsPropertyValue')?.value
              matchedField = integrationList?.candidateList?.find(field => field.id === selectedKey);
            }
            this.setBullhornCustomFieldLookups(matchedField);
            break;
          }
          default:
            break;
  ...
  private bindMergeTagsToOptions() {
    ...
    if (isAnyTagNull && isAdvanceTagActive) {
      this.toast.show("Some Mereg Tags for Options are not created !", { status: 'warning' });
    }
  }
  ...
  copyToClipboard(value: any) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(value).then(() => this.toast.show("Merge Tag Name Copied", { status: 'success' }))
        .catch(() => { this.toast.show("Error Copying Merge Tag Name  to clipboard.", { status: 'error' }) })
    } else {
      ...
      try {
        ...
        if (successful) {
          this.toast.show("Merge Tag Name Copied", { status: 'success' })
        }
        else {
          this.toast.show("Error Copying Merge Tag Name  to clipboard.", { status: 'error' });
        }
      } catch (error) {
        this.toast.show("Error Copying Merge Tag Name  to clipboard.", { status: 'error' })
      }
    ...
    if (this.currentCat.catId == 6 && this.contractThroughMergeTags) {
      ...
      if (modifyAtIndex != -1) {
        if (isSignReq) {
          ...
          this.propList[modifyAtIndex].attr.userGuideTxt = "Upload a Word document containing a signature merge tag (if applicable) for the user to view and sign. Please contact your Admin for the list of available merge tags that can be used in your document.";
        } else {
          this.propList[modifyAtIndex].attr.fileAccept = ".pdf,.docx";
          this.propList[modifyAtIndex].attr.userGuideTxt = "Upload a Word or PDF document containing a signature merge tag (if applicable) for the user to view and sign. Please contact your Admin for the list of available merge tags that can be used in your document.";
        }
      }
    }
  }
  ...
  private Validate_IAgreeButton(): boolean {
    ...
    if (fileDetail == null && defaultValue == null && uploadDocument == true && isDynamicAgreement == false) {
      this.toast.show('Please Upload a File', { status: 'error' });
      return false;
    }
    ...
    if (fileExt === 'pdf' && sign && this.contractThroughMergeTags) {
      this.toast.show('.pdf file is not allowed for signature', { status: 'error' });
      return false;
    }
