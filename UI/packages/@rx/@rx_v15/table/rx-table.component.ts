import {
  AfterContentInit, Component, ContentChild, ContentChildren, EventEmitter, Inject, Input, OnChanges, Output, QueryList, SimpleChanges
} from "@angular/core";
...
export class RxTableComponent extends DataCollection implements AfterContentInit, OnChanges {
  ...
  ngAfterContentInit(): void {
    ...
    var defaultFilterColumn = this.columns['_results'].filter(a => a.defaultFilter != "" && a.defaultFilter);
    if (!this.firstTimeBind && this.dataSource) {
      this.bindData(this.dataSource);
    }
    if (defaultFilterColumn.length > 0) {
      for (let index = 0; index < defaultFilterColumn.length; index++) {
        var column = defaultFilterColumn[index];
  ...
  showFilter(column: RxColumnComponent, event: any) {
    ...
    if ((result.left - width) < 300)
      column.isOpenLeftFilter = false;
    else
  ...
  filterColumn(column: RxColumnComponent) {
    if (column.defaultFilter)
      column.filterValue = column.defaultFilter;
