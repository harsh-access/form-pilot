...
export class RxSelectComponent extends Multilingual implements AfterContentInit, OnDestroy, OnChanges {
  @ViewChild("selectSearch", { static: true, read: ElementRef }) selectSearch: ElementRef;
  ...
  selectId: number;
  constructor(public elementRef: ElementRef, public renderer: Renderer2, public http: RxHttp, @Inject(API_HOST_URI) hostUri: string) {
    super();
  ...
  @Input() minimumCharacterSearchLength: number = 0;
  ...
  showItemBox() {
    if (this.minimumCharacterSearchLength == 0) {
      this.dropSource = [];
      this.setMainSource(this.userSource);
    }
  ...
  onBlur(event: any, value: string): void {
    ...
    if (!this.setValue(true, "value") || value == "") {
      ...
      this.blur.emit({ item: this.selectedTag, text: value, sourceItem: this.getSourceItem(this.selectedTag) });
  }
  ...
  onKeyup(event: KeyboardEvent, scroller: HTMLElement): void {
    ...
    if (value) {
      if (event.key !== "ArrowUp" && event.key !== "ArrowDown" && event.key !== "Enter" && event.key !== "ArrowLeft" && event.key !== "ArrowRight") {
        if (value.length >= this.minimumCharacterSearchLength) {
          if (this.autoComplete) {
            this.currentValue = value;
            this.onFocus(event);
          } else {
            ...
            if (this.noOfRecords)
              this.dropSource = this.dropSource.slice(0, this.noOfRecords)
            this.setActiveTabClass();
          }
      ...
      this.dropSource = [];
      this.changed.emit({ item: undefined, text: value, sourceItem: undefined })
    }
  }
  ...
  getSourceItem(tag: SelectModel) {
    var sourceItem;
    if (tag != undefined) {
      sourceItem = this.userSource.filter(t => t[this.keyValueProps[1]] == tag.key)[0];
    }
    return sourceItem;
  }
