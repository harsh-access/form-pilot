...
export class RxMaskDirective implements AfterViewInit, OnDestroy, ControlValueAccessor {
    ...
    ngAfterViewInit() {
        this.requiredControl = this.required;
    ...
    requiredView(e) {
        if (this.requiredControl) {
            if (e.target.value == "" || e.target.value == "___-___" || e.target.value == null || e.target.value == undefined) {
                this.required = true;
            }
            else {
                this.required = false;
