...
export class RxTagComponent extends Multilingual implements AfterContentInit, OnDestroy, OnChanges {
  ...
  @Input() splitChar: string = undefined;
  ...
  ngAfterContentInit(): void {
    ...
    if (this.splitChar != undefined) {
      this.splitCharacter = this.splitChar;
    }
  ...
  if(this.selectedTags.count()==0 && value){
  this.setSelectedTags(this.mainSource,this.currentValue)
  }
  }
  ...
  onKeydown(event: KeyboardEvent, scroller?: HTMLElement): void {
    if (event.keyCode === 40) {
      ...
      if (this.dropSource.length !== this.dropIndex) {
        ...
        this.changeScroll(event, scroller);
        if (this.openDrop === "")
          this.setActiveTabClass();
      }
    }
  ...
  private changeScroll(event: KeyboardEvent, scroller: HTMLElement): void {
    debugger
    var scrollerElement = document.getElementById(String(this.tagId));
    if (scrollerElement) {
      if (this.dropIndex > 6) {
        scrollerElement.scrollTo({
          top: event.key === "ArrowDown" ? scrollerElement.scrollTop + 32 : scrollerElement.scrollTop - 28,
          behavior: "smooth"
        });
      }
      else {
        scrollerElement.scrollTo({
          top: event.key === "ArrowUp" ? scrollerElement.scrollTop = 0 : null,
          behavior: "smooth"
        });
      }
  ...
  private clearValue(element: any): void {
    element.value = "";
    this.renderer.selectRootElement(element);
  }
